...
const PatientInfoForm = () => {
    ...
    const handleChange = (e) => {
        if (e.target.name === 'age') {
            // Validate age input to be between 1 and 100
            const ageValue = parseInt(e.target.value);
            if (ageValue >= 1 && ageValue <= 100) {
                setPatientInfo({ ...patientInfo, [e.target.name]: ageValue });
            } else {
                // Display an error message or handle invalid input
                // For now, let's set age to an empty string to indicate invalid input
                setPatientInfo({ ...patientInfo, [e.target.name]: '' });
            }
        } else {
